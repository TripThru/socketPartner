...
describe('Cabify state machine trip life cycle tests', function(){
  this.timeout(360000);
  
  before(function(){
    store.clear();
    localNetworkConfig.simulationType = 'cabify';
    foreignNetworkConfig.simulationType = 'cabify';
  ...
  it('should receive dropped_off status update from foreign network when driver reaches pickup location', function(done){
    tripLifeCycle.verifyDroppedOffStatus().then(done);
  });
